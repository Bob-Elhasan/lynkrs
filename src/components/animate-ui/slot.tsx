...
export function Slot<T extends HTMLElement = HTMLElement>({ children, ref, ...props }: SlotProps<T>) {
  const isAlreadyMotion =
    typeof children.type === "object" && children.type !== null && isMotionComponent(children.type);

  // motion.create() is memoized on [isAlreadyMotion, children.type], so identity is
  // stable across re-renders — the react-hooks static-components rule can't see that.
  ...
  // eslint-disable-next-line react-hooks/static-components -- Base is memoized above
